...
function eurosACentimos(euros: number) {
...
const formatearEurosFlecha = (centimos: number) => `${(centimos / 100).toFixed(2)} €`;
...
function aplicarIva(base: number, tipo: number = 21) {
    return Math.round(base + (base * tipo / 100));
}
...
function describirProducto(nombre: string, fabricante?: string) {
...
// en 1.5 SÍ?
// → Porque cuando se usa un valor predeterminado, no hace falta comprobar si es undefined porque nunca lo va a ser.
